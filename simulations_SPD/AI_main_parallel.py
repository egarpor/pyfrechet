...
def task(file) -> None:
    ...
    results = {'train_indices': train_idx.tolist(),
               ...
               'train_predictions': forest.predict_matrix(X_train).data.tolist(),
               'y_test_data': y_test.data.tolist(),
               'test_predictions': forest.predict_matrix(X_test).data.tolist(),
               'oob_errors': forest.oob_errors_matrix().tolist()}

    filename = 'AI_' + file[:-4] + '_block_' + str(current_block) + '_results.json'
...
print([file for file in   os.listdir(os.path.join(os.path.dirname(__file__), 'data'))[n_cores*(current_block-1):n_cores*(current_block)]])
